...
import {createTaskAC, deleteTaskAC, changeTaskTitleAC, changeTaskStatusAC} from "./action/tasks-actions.ts";
...
test('correct task should change its title', () => {
    const endState = tasksReducer(
        startState,
        changeTaskTitleAC({ todolistId: 'todolistId2', taskId: '1', title: 'kino' })
    )

    expect(endState.todolistId1[0].title).toBe('CSS')
    expect(endState.todolistId2[0].title).toBe('kino')
})